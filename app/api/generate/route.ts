...
${toneInstruction}${povInstruction}Requirements:
  ...
  - Use standard HTML tags such as <h2>, <p>, <a>, <ul>, and <li> as needed.
  - Only use <h2> for section headings. Do not use <h1>, <h3>, <h4>, <h5>, or <h6>.
...
`.trim();
    ...
    if (lengthOption === 'default') {
      sectionInstruction = 'Include around 9 <h2> headings. Only use <h2> for section headings.';
    } else if (lengthOption === 'custom' && customSections) {
      sectionInstruction = `Use exactly ${customSections} <h2> headings. Only use <h2> for section headings.`;
    } else if (sectionRanges[lengthOption || 'medium']) {
      const [minS, maxS] = sectionRanges[lengthOption || 'medium'];
      sectionInstruction =
        `Include ${minS}–${maxS} <h2> headings. Only use <h2> for section headings.`;
    } else {
      sectionInstruction = 'Include at least three <h2> headings. Only use <h2> for section headings.';
    }
